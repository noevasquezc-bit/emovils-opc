...
def schedule_followup_sequence(wa_number, nombre, fecha_viaje):
    try:
        ...
        if dias_restantes > 3:
            calendario.append({"tipo": "dia_3", "fecha_envio": str(hoy + timedelta(days=3))})
        if dias_restantes > 7:
            calendario.append({"tipo": "dia_7", "fecha_envio": str(hoy + timedelta(days=7))})
        if dias_restantes > 35:
            fecha_30 = viaje - timedelta(days=30)
            if fecha_30 > hoy:
                calendario.append({"tipo": "30_dias_antes", "fecha_envio": str(fecha_30)})
        if dias_restantes > 8:
            fecha_7 = viaje - timedelta(days=7)
            if fecha_7 > hoy:
                calendario.append({"tipo": "7_dias_antes", "fecha_envio": str(fecha_7)})
        if dias_restantes > 2:
...
def send_followup_message(wa_number, nombre, tipo):
    if tipo not in MENSAJES_SEGUIMIENTO:
        return {"status": "error", "error": "Tipo desconocido: " + tipo}
    template = MENSAJES_SEGUIMIENTO[tipo]
    mensaje = template.replace("{nombre}", nombre) if "{nombre}" in template else template
    send_text(wa_number, mensaje)
    lead = get_lead_by_whatsapp(wa_number)
    if lead:
        update_lead_status(lead["id"], LeadStatus.SEGUIMIENTO, "Seguimiento: " + tipo)
